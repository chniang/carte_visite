...
module.exports = async (req, res) => {
  ...
  // Plans mis a jour avec tarifs 2025
  const plans = {
    annual:   { name: 'CarteViz Annuel',   amount: 10000 },
    lifetime: { name: 'CarteViz A Vie',    amount: 25000 },
    business: { name: 'CarteViz Business', amount: 75000 }
  };
  const selectedPlan = plans[plan];
  if (!selectedPlan) return res.status(400).json({ error: 'Plan invalide. Plans disponibles : annual, lifetime, business' });
  ...
  try {
    const pdRes = await fetch(baseUrl + '/checkout-invoice/create', {
      ...
      body: JSON.stringify({
        invoice: {
          items: { item_0: { name: selectedPlan.name, quantity: 1, unit_price: selectedPlan.amount, total_price: selectedPlan.amount, description: 'Activation plan ' + plan + ' - ' + slug } },
          total_amount: selectedPlan.amount,
          description: selectedPlan.name + ' pour ' + slug
        },
        store: {
          name: 'CarteViz',
          tagline: 'Cartes de visite numeriques',
          phone: '+221776362714',
          postal_address: 'Dakar, Senegal',
          website_url: 'https://chniang.github.io/carte_visite/'
        },
        actions: {
          cancel_url: 'https://chniang.github.io/carte_visite/paiement.html?slug=' + slug,
          return_url: 'https://chniang.github.io/carte_visite/merci.html?slug=' + slug + '&plan=' + plan,
          callback_url: 'https://carteviz-backend.vercel.app/api/ipn-callback'
        },
